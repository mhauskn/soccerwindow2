...
MonitorClient::handleReceive()
{
    ...
    if ( M_version >= 3 )
    {
        ...
        while ( M_socket->hasPendingDatagrams() )
        {
            ...
            {
                if ( M_version == 4
                     && ! M_view_holder.addDisp4( buf ) )
                {
                    std::cerr << "recv: " << buf << std::endl;
                }
                else if ( M_version == 3
                          && ! M_view_holder.addDisp3( buf ) )
                {
                    std::cerr << "recv: " << buf << std::endl;
                }
    ...
    else if ( M_version == 2 )
    {
        ...
        while ( M_socket->hasPendingDatagrams() )
        {
            ...
            if ( n > 0 )
            {
                if ( ! M_view_holder.addDispInfo2( disp2 ) )
                {
                    std::cerr << "recv: "
    ...
    else if ( M_version == 1 )
    {
        ...
        while ( M_socket->hasPendingDatagrams() )
        {
            ...
            if ( n > 0 )
            {
                if ( ! M_view_holder.addDispInfo( disp1 ) )
                {
                    std::cerr << "recv: "
...
MonitorClient::handleTimer()
{
         ...
         && M_waited_msec >= Options::instance().waitSeconds() * 1000 )
    {
        ...
        {
            std::cerr << "waited " << M_waited_msec * 1000 << " seconds.\n"
                      << "end..."
                      << std::endl;
